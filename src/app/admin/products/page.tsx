...
import { 
  ...
  X, Image as ImageIcon, Store, Tag, Filter, Check
} from "lucide-react";
...
export default function AdminProductsPage() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const { data, error } = await supabase.from('products')
        .select(`
          id, title, title_te, slug, price, compare_at_price, status, approval_status, rejection_reason, submitted_at, created_at, brand, sku, description, stock_quantity, weight_kg, parcel_weight_kg,
  ...
  return (
    <AdminLayout title="Product Approvals & Catalog Control" subtitle="Review submitted products, verify pricing, enforce marketplace standards, and publish to catalog">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse text-xs">
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  <tr><td colSpan={7} className="p-8 text-center text-slate-400">Loading products...</td></tr>
                  ...
                  filteredProducts.map((p) => {
                    ...
                    return (
                      <tr key={p.id} className="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td className="p-4 font-semibold text-slate-700">{p.store?.name || '-'}</td>
                        <td className="p-4 text-slate-600">{p.category?.name || 'General'}</td>
                        ...
                        <td className="p-4 text-slate-400">{p.submitted_at ? new Date(p.submitted_at).toLocaleDateString() : '-'}</td>
          ...
          <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col border border-slate-200">
              <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                <h3 className="font-black text-slate-900 text-sm">Product Inspection</h3>
              ...
              <div className="p-6 overflow-y-auto space-y-4 text-xs">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-slate-50 p-3 rounded-xl">
                  ...
                  <div>
                    <span className="text-slate-400 block font-semibold">MRP Price</span>
                    <span className="font-bold text-slate-700">{selectedProduct.compare_at_price ? formatCurrency(selectedProduct.compare_at_price) : '-'}</span>
                  </div>
                  <div>
                ...
                <div>
                  <span className="text-slate-400 block font-semibold mb-1">Description</span>
                  <p className="p-3 bg-slate-50 rounded-xl text-slate-700 leading-relaxed">{selectedProduct.description || 'No description provided.'}</p>
                </div>
