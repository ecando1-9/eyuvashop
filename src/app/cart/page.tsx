...
import { CartItem, useCartStore, useSyncCart } from '@/hooks/useCartStore';
...
export default function CartPage() {
  ...
  const updateQuantity = async (item: CartItem, newQuantity: number) => {
    ...
    await updateCartQuantity(item.product.id, newQuantity, item.selectedVariantId);
  };

  const removeItem = async (item: CartItem) => {
    await removeCartItem(item.product.id, item.selectedVariantId);
  };
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="flex-1 space-y-4">
                {cartItems.map((item) => {
                  ...
                  const primaryImage = product?.images?.find((img: { is_primary?: boolean }) => img.is_primary)?.url || product?.images?.[0]?.url;
                  const isOutOfStock = typeof product?.stock_quantity === 'number' && product.stock_quantity < item.quantity;
                  const isNotPublished = product?.status ? product.status !== 'published' : false;
                  const itemKey = `${product.id}-${item.selectedVariantId || 'default'}`;

                  return (
                    <div key={itemKey} className="flex flex-col sm:flex-row bg-white border rounded-lg p-4 gap-4 shadow-sm relative">
                      {(isOutOfStock || isNotPublished) && (
                        <div className="absolute top-0 left-0 w-full h-full bg-white/60 backdrop-blur-[1px] z-10 flex items-center justify-center pointer-events-none">
                        ...
                        <div className="flex items-center justify-between mt-4">
                          <div className="flex items-center border rounded-md">
                            ...
                            <button 
                              onClick={() => updateQuantity(item, item.quantity + 1)}
                              className="p-2 hover:bg-gray-100 disabled:opacity-50"
                              disabled={typeof product?.stock_quantity === 'number' && product.stock_quantity <= item.quantity}
                            >
